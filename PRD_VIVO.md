...
| Última atualização | 2026-08-18 por Claude Code (`fix/landing-publica` — "/" pública com landing placeholder + botão de login na navbar; ver §6) |
...
| KAN-110: Landing page                    | 🚧     | Claude | `fix/landing-publica` (2026-08-18) — `/` agora é pública e renderiza `landing-page.tsx` (placeholder mínimo: navbar + hero + 3 cards de recursos, componentes do design system, sem `@radix-ui`). A landing "cena Three.js" (efeito ASCII, samurai procedural, mergulho de câmera por scroll) descrita na issue original **nunca existiu neste repositório em nenhum formato** — auditoria confirmou (grep + PRD já marcava isso como pendente). Virou task 8.12 no BACKLOG, fora do escopo deste fix (decisão do Rafael) |
