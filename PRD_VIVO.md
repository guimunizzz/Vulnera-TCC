...
| Métrica            | Valor                               |
| ------------------ | ----------------------------------- |
| Sprint atual       | **Fase 9 — Findings Globais + Query Wizard** (branch `feat/findings-globais`; CP0-CP9 concluídos e validados no navegador em 2026-09-14, **não commitada**) |
| Data início        | 2026-06-10 (Sprint 0)               |
| Data alvo TCC      | 2026-10-25 (**~6 semanas restantes** — 41 dias a partir de 2026-09-14; o valor anterior dizia 11 semanas e estava desatualizado) |
| Última atualização | 2026-09-14 por Claude Code (**FEAT-09**: listagem global de findings com filtros/facetas/paginação no banco, query wizard com caixa de sugestões, exportação CSV e componente canônico de tabela; fecha `BACKEND-002`; ver §3 e §6) |
...
### FEAT-09 — Findings Globais + Query Wizard (Fase 9)

> Página global de findings para PENTESTER e ADMIN, com filtros no estilo query
> wizard, e unificação das duas listagens que existiam em paralelo.
> Fecha o finding **`BACKEND-002`** da auditoria consolidada
> (*"controller aceita apenas `projectId`; queries não têm
> severity/status/OWASP/skip/take"*). Decisão em **ADR-028**.

| Task                                             | Status | Owner | Notas |
| ------------------------------------------------ | ------ | ------ | ----- |
| KAN-901: `GET /api/vulnerabilities` com filtros   | ✅     | Claude | projectId/applicationId/companyId/createdBy/severity/status/owaspCategory (repetíveis **e** separados por vírgula) + search + createdFrom/To + page/pageSize/sortBy/sortOrder. AND entre campos, OR dentro do campo |
| KAN-902: Paginação e facetas no banco             | ✅     | Claude | `groupBy` por severidade/status/OWASP/empresa; cada faceta ignora o próprio filtro e respeita os demais. `pageSize` clampado em 100, nunca recusado |
| KAN-903: Ordenação por ranking de severidade      | ✅     | Claude | `severityFinal` é String (sem enums nativos), então `orderBy` daria ordem alfabética com MEDIUM depois de LOW. Resolvido com `FIELD()` do MySQL em `$queryRaw` parametrizado; gêmeo do `where` do Prisma, travado pelo canário VULN-LIST-07b |
| KAN-904: Leitura do AuditLog                      | ✅     | Claude | O `AuditLog` era append-only **e write-only** — só existia `create()`. `GET /vulnerabilities/:id/audit-log` + `AuditLogEntity`; mesma regra de visibilidade do finding. Sem migration |
| KAN-905: Testes de backend                        | ✅     | Claude | `vulnerability-search.test.ts` — VULN-LIST-01..14, TEN-19..22, AUD-01..03 (23 canários). Suíte 269 → **293**. Cobertura do service: 96,3% stmts / 99,1% linhas |
| KAN-906: Parser do query wizard                   | ✅     | Claude | `lib/finding-query.ts` — puro, zero React, **nunca lança**. Operadores `=` `!=` `~` `:`; campos PT-BR com e sem acento + apelidos em inglês; `!=` vira o complemento do conjunto no cliente. **108 testes**, incluindo digitação caractere a caractere |
| KAN-907: Componente canônico de tabela            | ✅     | Claude | `components/findings/findings-table.tsx` + `use-findings.ts` + `use-findings-filters.ts`. `Table` do design system estendida (aditivo) com ordenação controlada e linha operável por teclado |
| KAN-908: Página `/findings` + detalhe enriquecido | ✅     | Claude | Rota `ADMIN`/`PENTESTER` + item na Sidebar; URL compartilhável. `/findings/:id` já era rota de topo — ganhou trilha Empresa›Aplicação›Projeto, CVSS decomposto nas 8 métricas, bloco calculado×aplicado×justificativa, trilha de auditoria e ações |
| KAN-909: Remoção da listagem antiga               | ✅     | Claude | Aba do ProjectDetail (~120 linhas: 3 `<select>` nativos, `<table>` crua, filtro/paginação em memória) substituída. Migrados também os 3 dashboards e o `app/mobile` |
| KAN-910: Documentação                             | ✅     | Claude | `docs/FINDINGS_QUERY.md` (guia de uso), ADR-028, `docs/BACKLOG.md` §Findings de auditoria, nota de módulo no vault, Changelog |
| KAN-911: Caixa de sugestões na barra de busca     | ✅     | Claude | `query-suggestions.tsx` + `analisarContexto`/`aplicarSugestao` no parser. Abre no clique mostrando os 7 campos com descrição e exemplo; escolher um campo emenda `=` e passa a mostrar os VALORES daquele campo com a contagem da faceta. Padrão APG combobox+listbox (foco nunca sai do input, `aria-activedescendant`). ⚠️ só oferece campo que consegue completar: `empresa` só para ADMIN, `aplicacao` só para ADMIN/CLIENT — evita 403 e chip natimorto |
| KAN-912: Exportação CSV do recorte                | ✅     | Claude | `findings-csv.ts` (puro) + `use-findings-export.ts`. Exporta o RECORTE inteiro (pagina de 100 em 100), não a página visível; sem filtro exporta tudo. Montado no navegador, como os PDFs (ADR-003). `;` + BOM para o Excel pt-BR; **neutraliza injeção de fórmula** (`=`/`+`/`-`/`@`) |
| KAN-913: Correção de espaçamento fora da escala   | ✅     | Claude | 9 classes que não existem na escala CUSTOM do projeto e portanto **não emitiam CSS nenhum** — `gap-1.5` (a severidade colada no número que o Rafael apontou), `mt-1.5`, `w-64`, `w-96`, `w-48`, `h-32`, `h-40`, `h-64`. Resumo de severidade virou `<dl>` com espaçamento assimétrico (`gap-2` dentro do par, `gap-x-6` entre pares) |

**Mudanças de contrato** (todas com consumidores migrados na mesma entrega):

- `GET /api/vulnerabilities` devolve `{ data, pagination, facets }` em vez de `Vulnerability[]`. 6 asserções de teste migradas; `?projectId=` inexistente segue **404** e projeto alheio segue **403**.
- `GET /api/vulnerabilities/:id` ganhou `projectName`/`applicationName`/`companyName` (**aditivo**) — sem isso a trilha ficaria incompleta justamente para o PENTESTER, que não acessa o endpoint de aplicações.

⚠️ **Decisão de Rafael em 2026-09-14:** o prompt original pedia **403 para CLIENT**
neste endpoint. A auditoria mostrou que ele já existia, já servia CLIENT (RN16) e
tinha 4 consumidores vivos — os três dashboards e o app mobile, que é o app do
CLIENT. Decidido manter o acesso do CLIENT ao endpoint (escopado à própria
...
| 2026-09-14 | Build Docker do `web` destravado na branch `feat/nova-landing` | `docker compose up --build` quebrava em `npm run build` (8 erros TS2305/TS7006: `screen`, `waitFor` e `within` "inexistentes" em @testing-library/react). Causa: `--legacy-peer-deps` (necessário — sem ele o npm 10.9 do node:22-alpine estoura com o bug do Arborist `edgesOut`) desliga a auto-instalação de peers, e `@testing-library/dom` nunca fora declarado — só existia localmente porque o npm do dev a instalava sozinho. Fix: `@testing-library/dom@^10.4.1` em devDependencies do `app/web` + comentários do Dockerfile corrigidos. Validado: build `--no-cache` verde, stack inteira `up` (db/api/web healthy), web 200 e `/api/health` 200, `npm run check` 34/34 |
| 2026-09-14 | **Fase 9 — Findings Globais + Query Wizard concluída** | Branch `feat/findings-globais`, **não commitada** (o prompt pediu explicitamente para não commitar nem abrir PR). **Fecha o finding `BACKEND-002`** da auditoria consolidada — a listagem de findings não tinha filtro nenhum no servidor, e era por isso que existiam DUAS implementações de listagem no frontend, cada uma filtrando em memória do seu jeito (ver ADR-028). **Backend:** `GET /api/vulnerabilities` ganhou 12 parâmetros de filtro (repetíveis e separados por vírgula), paginação e **facetas contadas por `groupBy`** — cada faceta ignora o próprio filtro e respeita os demais, que é o que permite marcar uma segunda severidade depois da primeira. Ordenar por severidade exigiu `FIELD()` do MySQL em `$queryRaw` parametrizado: `severityFinal` é String (schema sem enums nativos), então `orderBy` do Prisma daria ordem ALFABÉTICA, com MEDIUM depois de LOW — o canário VULN-LIST-07 fixa isso, e o VULN-LIST-07b amarra os dois construtores de filtro gêmeos. Descoberto no caminho que o **`AuditLog` era append-only E write-only**: gravado desde a Fase 5 e nunca lido por ninguém, só acessível abrindo o MySQL — ganhou caminho de leitura (`GET /vulnerabilities/:id/audit-log`), sem migration. **Frontend:** parser puro do query wizard (`lib/finding-query.ts`, 108 testes, **nunca lança** — a barra é reparsada a cada tecla) e UM componente de tabela (`findings-table.tsx`) configurado por contexto, substituindo as duas implementações. A `Table` do design system ganhou ordenação controlada (obrigatória com paginação no servidor: ordenar no cliente reordenaria só as 25 linhas da página — parece que funcionou e está errado) e linha operável por teclado. **Migrados junto:** aba do ProjectDetail (~120 linhas removidas), os 3 dashboards (que baixavam TODOS os findings para contar cinco números) e o `app/mobile`. **Validação no navegador real: a extensão do Chrome CONECTOU desta vez** (Rafael logou no meio da sessão) — os 12 itens do CP8 conferidos, console com 0 erros; Playwright ad-hoc usado só onde a extensão não alcança (emular `prefers-reduced-motion` e contar requisições). **4 bugs meus achados na validação e corrigidos**, todos invisíveis para os testes automatizados: (1) resumo do cabeçalho não fechava com o total, porque reusava a faceta drill-down; (2) chip inválido sumia antes de a pessoa ler o motivo, porque a URL reescrevia o rascunho depois do debounce; (3) estado VAZIO aparecia no lugar do de ERRO com a API fora — a tela dizia 'Nenhum finding com esses filtros' e a pessoa mexeria no filtro à toa; (4) **'Tentar novamente' não disparava requisição nenhuma**: o `networkMode: "online"` padrão do TanStack Query não transforma falha de rede em erro — PAUSA a consulta (`status: pending`, `error: null`), e `refetch()` numa consulta pausada também pausa; só recarregar a página saía do estado. Corrigido com `networkMode: "always"` (diagnosticado contando requisições de rede, com `navigator.onLine === true`). **Testes: 269 → 293 no backend** (VULN-LIST-01..14, TEN-19..22, AUD-01..03; cobertura do service 96,3% stmts / 99,1% linhas) e **34 → 146 no frontend** (108 do parser + 4 da guarda de acesso). Lint, contraste 66/66, `tsc` nos 3 workspaces e `vite build` verdes |
...
| 2026-09-14 | **Uma queda da API desloga o usuário.** Com o backend fora do ar, o refresh token falha e o interceptor de `lib/api/client.ts` chama `clearAuth()`, mandando a pessoa para o login | Experiência em instabilidade de rede | R | Aberto — pré-existente, encontrado ao validar o estado de erro da Fase 9. Distinguir "refresh recusado" (401 legítimo, deve deslogar) de "refresh não chegou ao servidor" (falha de rede, não deve) resolveria |
| 2026-09-14 | **`networkMode: "online"` do TanStack Query silencia falhas de rede no app inteiro.** A consulta pausa em vez de errar, então a tela não mostra erro nem oferece recuperação. Corrigido só nas buscas de finding (`use-findings.ts`), com `[FUTURO]` no código | Todas as telas fora da Fase 9 | R | Aberto — mudar o `queryClient` global resolve de uma vez, mas afeta telas fora do escopo desta entrega |
| 2026-09-14 | Canário `TEN-18` nunca existiu — a numeração de tenancy pula de `TEN-17` (Fase 6.5) para `TEN-19` (Fase 9), seguindo o enunciado do prompt | Leitura da suíte de testes | R | Aberto — cosmético; renumerar quebraria a rastreabilidade dos canários antigos |
