...
//
// Rotas do módulo DAST — TODAS exigem autenticação limitada + requireRole("PENTESTER","ADMIN").
...
router.use(authRateLimitMiddleware, requireRole("PENTESTER", "ADMIN"));

router.post("/", endpointRateLimitMiddleware("dast"), (req, res) => controller.create(req, res));
...
router.get("/:id/report/html", endpointRateLimitMiddleware("report"), (req, res) => controller.getReportHtml(req, res));
router.get("/:id/report/data", endpointRateLimitMiddleware("report"), (req, res) => controller.getReportData(req, res));
